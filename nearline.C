...
void nearline(int runnum, TString settingname, int port)
{
    ...
    //TString filename = "--stream=lxir123:7803";
      // filename = Form("/lustre/land/202002_s467/stitched/main%04d_*.lmd",runnum);
      //filename = Form("/u/taniuchi/s467/s467_lmd/main%04d_*.lmd",runnum);
      filename = Form("/u/taniuchi/s467/s467_lustertmp/main%04d_*.lmd",runnum);
      // TString filename = "/lustre/land/202002_s467/stitched/main0235_0001.lmd";
    }
    TString outputFileName = Form("./rootfiles_local/data_s467_%04d.root",runnum);
    // Output file ------------------------------------------
    Bool_t fCal_level_califa = true;  // set true if there exists a file with the calibration parameters
    Bool_t NOTstoremappeddata = true;// true; // if true, don't store mapped data in the root file
    ...
    if (fMusic && fCalifa && fTwim)
    {
        ...
        CalifaAmsOnline->SetZproj(20.0);//36.);//(20.0); // Projectile atomic number
