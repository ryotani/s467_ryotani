...
void scaler_online(int runnum)
{
    ...
    const Int_t nev = 500000; // Only nev events to read
	...
	sofiacalfilename = sofiacaldir + "CalibParam_highgain_FRS" + to_string(FRSsetting[i]) + ".par";
      ...
      outputFilename = Form("./rootfiles/s467_scaler_Setting%i_%04d_%i%s.root", FRSsetting[i], runnum, datime->GetDay(), month.c_str());
    ...
    R3BTrloiiScalerReader_s467* unpacktrlo;
	...
	unpacktrlo =
  	    new R3BTrloiiScalerReader_s467((EXT_STR_h101_TRLO_s467_onion*)&ucesb_struct.trlo, offsetof(EXT_STR_h101, trlo));
    }
    // Add readers ------------------------------------------
	...
	//
	unpacktrlo->SetOnline(false);
	source->AddReader(unpacktrlo);
    }
    // Create online run ------------------------------------
	...
	*/
	...
	scalersonline->SetReadTrloii(true);
