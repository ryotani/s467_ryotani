...
TString inpar = sofiacaldir + "FRS13.par";
TString outpar ="./fragment/out.par";
//
bool IsEmpty=false;
...
void tofw_beta_offset_nofrsgate(int i_target = 0);
...
void define_conditions(){
  ...
  for(int i = MINPADDLE; i<NUMPADDLE; i++){
    ...
    ch->Draw(Form("TwimTheta*1000.:(FRSAoQ-FragAoQ)>>h_aoqdiff_twim%i",i),Form("Tofw_Paddle==%i && %s && %s",i+1, cut_mw.Data(), Zgate.Data()),"colz");
    h->Write();
  ...
  */
  ...
  // Angle change
    ...
    ch->Draw(Form("(TwimTheta-MusicTheta)*1000.:(FRSAoQ-FragAoQ)>>h2_%i",i),Form("Tofw_Paddle==%i && %s && %s",i+1, cut_mw.Data(), Zgate.Data()),"colz");
    h->Write();
    if(i==NUMPADDLE/2-1) c->Print(outpdf);
  }
  c->Print(outpdf );
  ...
  /* / Theta Tof
    ...
    ch->Draw(Form("(TwimTheta)*1000.:(FragTof)>>h4_%i",i),Form("Tofw_Paddle==%i && %s && %s",i+1, cut_mw.Data(), Zgate.Data()),"colz");
    h->Write();
    if(i==NUMPADDLE/2-1) c->Print(outpdf);
  }
  c->Print(outpdf );
  ...
  Double_t peakX[NUMPADDLE]={1000.};
    ...
    //ch->Draw(Form("(FRSAoQ-FragAoQ)>>h_noreaction_%i",i),Form("Tofw_Paddle==%i && %s && %s",i+1, cut_mw.Data(), Zgate.Data()),"colz");
    ch->Draw(Form("(FRSAoQ-FragAoQ)>>h_noreaction_%i",i),Form("Tofw_Paddle==%i && %s",i+1, Zgate.Data()),"colz"); // include mw0x<0
  ...
  for(int i = MINPADDLE; i<NUMPADDLE; i++){
    ...
    ch->Draw(Form("(TwimTheta)*1000.:(FragTof)>>h4_gated_%i",i),cut && Form("Tofw_Paddle==%i && %s && %s",i+1, cut_mw.Data(), Zgate.Data()),"colz");
    h->Write();
    if(i==NUMPADDLE/2-1) c->Print(outpdf);
  }
  c->Print(outpdf );
