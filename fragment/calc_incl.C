const int NUMTARGET=3, NUMPADDLE=28, MAXEVENT=-1;//10000000;
const int MINZ=10, MAXZ=30, MAXA=60, BINAOQ=500, BINZ=500;
const double MINAOQ=2.25, MAXAOQ=2.75;
//
TString dir_rootfile = "fragment/output/" ;
TString filename = "tofw_beta_offset_paddle_50Ca_TARG_nofrsgate_Feb.root"; // TARG will be replaced
TString outfilename = "incl_out.root";
...
void fill_histos(int i){
  ...
  for(int p =0; p<NUMPADDLE; p++){
    ...
    h_pid[i][p] = new TH2I(dummy,dummy,BINAOQ, MINAOQ, MAXAOQ, BINZ, MINZ, MAXZ);
    for(int j=0; j<MAXZ*MAXA; j++){
      dummy = Form("h_gated_pid_%i_%i_%i_%s", p+1, j/MAXA, j%MAXA, targetname[i].Data());
      h_gated_pid[i][p][j/MAXA][j%MAXA] = new TH2S(dummy, dummy, BINAOQ, MINAOQ, MAXAOQ, BINZ, MINZ, MAXZ);
    }
    get_mem_usage();
  }
  //
  ...
  for(int p =0; p<NUMPADDLE; p++)
    {
      ...
      if(h_music[i][p]->GetEntries()<numpeaks){
	...
	continue;
      ...
      f_music[i][p] = generate_func(Form("f_music_%s_%i",targetname[i].Data(),p), numpeaks, MINZ, 1, 0.3, h_music[i][p] ->GetMaximum());
	...
	f_music[i][p] ->SetParameter(3*j+3,0.2);
      ...
      //
      //f_music[i][p] ->SetParLimits(0,0.,h_music[i][p] ->GetMaximum());
...
void fill_aoq_gated(int i){
  ...
  for(int p =0; p<NUMPADDLE; p++){
	...
	TString dummy = Form("h_aoq_gated_%i_%i_%i_%i_%s", p+1, j/MAXA, j%MAXA, k, targetname[i].Data());
  ...
  for(Long64_t n=0; n<nentry[i]; n++){
    ...
    h_aoq_gated[i][p][tmpZ][tmpA][tmpZ_frag]->Fill(FragAoQ);
  }
  cout<<endl;
  //
  fout->cd();
  ...
  for(int p =0; p<NUMPADDLE; p++){
    ...
    for(int ZA=MINZ*MAXA; ZA<MAXZ*MAXA; ZA++){//FRS
      for(int k =MINZ; k<MAXZ; k++){//Frag
	int rangeA[2] = {(int)(1.8*(double)k), (int)min(2.8*(double)k,(double)MAXA)};
	...
	auto f_tmp = generate_func(Form("f_aoq_%s_%i_%i_%i_%i",targetname[i].Data(),p, ZA/MAXA, ZA%MAXA,k), numpeaks,
			      (double)rangeA[0]/(double)k, 1./(double)k, 0.3/(double)k, 2*h_tmp->GetMaximum());
	cout<<"Paddle"<<p+1<<", Z"<<ZA/MAXA<<", A"<<ZA%MAXA<<", Zfrag"<<k<<endl;
	f_aoq[i][p][ZA/MAXA][ZA%MAXA][k] = f_tmp;
	f_tmp->FixParameter(0,0.);
	h_tmp->Fit(f_tmp,"QL","",(double)(rangeA[0]-1)/(double)k, (double)(rangeA[1]+1)/(double)k);
	//
	//f_tmp->SetParLimits(0,0.,h_tmp->GetMaximum());
	auto params = f_tmp->GetParameters();
	for(int peak=0; peak<numpeaks; peak++){
	  if(params[3*peak+1]>1.) continue;
	  f_tmp ->SetParameter(3*peak+1,0.);
	  f_tmp ->SetParameter(3*peak+2,0.);
	  f_tmp ->SetParameter(3*peak+3,0.1);
	  f_tmp ->FixParameter(3*peak+1,0.);
	  f_tmp ->FixParameter(3*peak+2,0.);
	  f_tmp ->FixParameter(3*peak+3,0.1);
	}
	h_tmp->Fit(f_tmp,"QL","",(double)(rangeA[0]-1)/(double)k, (double)(rangeA[1]+1)/(double)k);
	//
	cout<<"AoQ fit result for paddle"<<p+1<<", Z_frag"<<k<<". Chi-sq:"<<f_music[i][p]->GetChisquare()<<", NDF:"<<f_music[i][p]->GetNDF()<<endl<<endl;
	//
	for(int j=0; j<numpeaks; j++){
...
TF1* generate_func(char*name, int numpeaks, double minpos, double interval, double par_limit_range, double height_limit){
  ...
  for(int i=0; i<numpeaks; i++){
    formula.Append(Form("+gausn(%i)",3*i+1));
  }
  ...
  for(int i=0; i<numpeaks; i++){
    ...
    f_tmp->SetParameter(3*i+3, 0.5*par_limit_range);
...
Double_t pid(Double_t &zet, Double_t &aoq, bool isfrag=false, Int_t i, Int_t targ){
