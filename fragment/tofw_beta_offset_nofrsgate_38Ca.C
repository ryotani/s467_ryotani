...
const Int_t MINA=30, MAXA=44, MINZ=16, MAXZ=22;
//
...
void tofw_beta_offset_nofrsgate_38Ca();
...
void delta_beta_method(){
  ...
  //for(Long64_t n=0; n<100; n++){
    ...
    if(TMath::Abs(FRSAoQ-FragAoQ-f_aoq[i]->GetParameter(1)) > 2.*f_aoq[i]->GetParameter(2)  || TMath::Abs(FragZ-tmpzet)>0.4 ||
       TMath::Abs(TwimTheta-6e-3)*1000. > 14. || TMath::Abs(FragTof-40-0.4*(Double_t)i)>10.) continue; // ToF gate was +-2ns
